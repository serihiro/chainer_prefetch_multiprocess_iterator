...
class _PrefetchPipeline:
    ...
    def __init__(self,
        ...
        _prefetch_multiprocess_iterator_waiting_id_queue = multiprocessing.Queue(
            waiting_id_queue_max_size)
        _prefetch_multiprocess_iterator_waiting_id_queue.cancel_join_thread()
        _prefetch_multiprocess_iterator_cached_id_queue = multiprocessing.Queue(
            prefetched_id_queue_max_size)
    ...
    def launch_thread(self):
        ...
        self._start_generate_random_id_process_time = time.time(
        ) - start_generate_random_id_process_timer  # timer
...
def _prefetch_from_backend(
    ...
    while True:
        ...
        while not _prefetch_multiprocess_iterator_terminating.is_set():
            try:
                indices = _prefetch_multiprocess_iterator_waiting_id_queue.get(
...
def _generate_batch(inputs):
    ...
    backend_storage_file_path = os.path.join(
        _prefetch_multiprocess_iterator_fetch_dataset.root, path)
    local_storage_file_path = _solve_local_storage_path(_prefetch_multiprocess_iterator_local_storage_base,
                                                        backend_storage_file_path)
